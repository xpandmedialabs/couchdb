...
class XPCURLHTTPAdapter extends XPHTTPAdapter {
  private $ch;
  ...
  public function __construct($host, $port) {
    if(!extension_loaded('curl')) {
      throw new XPException('XPCouchDB cannot use cURL on this system: the PHP cURL extension is not installed.');
    }
  ...
  public function procPacket($method, $url, $data = null, $reqHeaders = array(), $specialHost = null, $specialPort = null) {
    ...
    if($chResponse !== false) {
      ...
      throw new XPException('cURL error #' . curl_errno($this->ch) . ': ' . curl_error($this->ch));
    }
    else {
      throw new XPException('cURL returned false without providing an error.');
    }
   ...
   * @return XPCURLHTTPAdapter Returns $this if talking to the same server
  ...
  private function makeFollowAdapter($parts) {
    ...
    $adapter = new XPCURLHTTPAdapter($parts['host'], $parts['port']);
