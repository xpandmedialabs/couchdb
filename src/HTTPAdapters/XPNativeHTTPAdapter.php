...
class XPNativeHTTPAdapter extends XPHTTPAdapter {
  ...
  public function useSSL($use) {
    throw new XPException('XPCouchDB::$HTTP_NATIVE_SOCKETS does not support SSL.');
  }
  ...
  public function procPacket($method, $url, $data = null, $reqHeaders = array(), $specialHost = null, $specialPort = null) {
    ...
    while(!$sock) {
      if(sizeof($this->connPool) > 0) {
        ...
        try {
          ...
          throw new XPException('Was unable to fsockopen() a new socket: ' . $e->getMessage());
        }
      }
    }

    if(!$sock) {
      throw new XPException("Error connecting to {$host}:{$port} - $sockErrStr ($sockErrNo).");
    }
    ...
    while(
      ...
      if($sockInfo['timed_out']) {
        throw new XPException('Connection timed out while reading.');
      }
      ...
      if(!$line && !$sockInfo['feof'] && !$sockInfo['timed_out']) {
        throw new XPException('Unexpectedly failed to retrieve a line from the socket before the end of the file.');
      }
      ...
      if($isHeader) {
        ...
        if($isHeader && empty($line)) {
          ...
          if(!isset($response->headers->_HTTP->raw)) {
            ...
            if(preg_match('(^HTTP/(?P<version>\d+\.\d+)\s+(?P<status>\d+))S', $line, $match)) {
              ...
              throw new XPException('There was a problem while handling the HTTP protocol.'); //whoops!
            }
          }
          else {
            $line = explode(':', $line, 2);
        ...
        if($chunkSize === null) {
          ...
          if(!empty($line) || $line == "0") {
            ...
            if(!is_int($chunkSize)) {
              throw new XPException('Invalid chunk size: '.$line);
            }
          }
        }
        else if($chunkSize === 0) {
          ...
          if(strlen($line) > $chunkSize && strlen($line) - 2 > $chunkSize) {
            throw new XPException('Unexpectedly large chunk on this line.');
          }
          else {
            $response->body .= $line;
          ...
          throw new XPException('Unexpected empty line.');
        }
      }
      else {
        /*
